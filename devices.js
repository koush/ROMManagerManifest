{
    "devices": [{
        "name": "Motorola Droid",
        "version": "2.5.0.1",
        "init": "init.sholes.rc",
        "alternate_recovery": {
            "name": "SPRecovery 0.99.3b",
            "url": "http://koush.kanged.net/cm/recoveries/sprecovery.img",
            "clockwork": false
        },
        "legacy_versions": ["3.1.0.2", "2.5.1.4", "2.5.1.3", "2.5.1.1", "2.5.0.1"],
        "key": "sholes"
    },
    {
        "name": "HTC MyTouch 3G",
        "version": "2.5.0.7",
        "init": "init.sapphire.rc",
        "alternate_recovery": {
            "name": "RA Recovery v1.7.0G",
            "url": "http://files.androidspin.com/downloads.php?dir=amon_ra/RECOVERY/&file=recovery-RA-sapphire-v1.7.0G.img",
            "clockwork": false
        },
        "legacy_versions": ["2.5.0.7", "2.5.1.4", "2.5.1.3", "2.5.1.1"],
        "key": "sapphire"
    },
    {
        "name": "HTC Magic",
        "version": "2.5.0.7",
        "init": "init.sapphire.rc",
        "alternate_recovery": {
            "name": "RA Recovery v1.7.0H",
            "url": "http://files.androidspin.com/downloads.php?dir=amon_ra/RECOVERY/&file=recovery-RA-sapphire-v1.7.0H.img",
            "clockwork": false
        },
        "legacy_versions": ["2.5.0.7", "2.5.1.4", "2.5.1.3", "2.5.1.1"],
        "key": "magic"
    },
    {
        "name": "HTC G1/Dream",
        "version": "2.5.0.7",
        "init": "init.trout.rc",
        "alternate_recovery": {
            "name": "RA Recovery v1.7.0",
            "url": "http://files.androidspin.com/downloads.php?dir=amon_ra/RECOVERY/&file=recovery-RA-dream-v1.7.0.img",
            "clockwork": false
        },
        "legacy_versions": ["2.5.0.7", "2.5.1.4", "2.5.1.3", "2.5.1.1"],
        "key": "dream"
    },
    {
        "name": "Google Nexus One",
        "version": "5.0.2.0",
        "init": "init.mahimahi.rc",
        "alternate_recovery": {
            "name": "RA Recovery v2.2.1",
            "url": "http://files.androidspin.com/downloads.php?dir=amon_ra/RECOVERY/&file=recovery-RA-passion-v2.2.1.img",
            "clockwork": false
        },
        "legacy_versions": ["3.0.0.5", "2.5.1.4", "2.5.1.3", "2.5.1.1", "3.0.0.5", "5.0.0.1", "5.0.0.8", "5.0.1.0"],
        "key": "passion",
        "lunch": "cyanogen_passion-eng"
    },
    {
        "name": "HTC Hero (GSM)",
        "version": "2.5.0.7",
        "init": "init.hero.rc",
        "alternate_recovery": {
            "name": "RA Recovery v1.7.0.1",
            "url": "http://files.androidspin.com/downloads.php?dir=amon_ra/RECOVERY/&file=recovery-RA-hero-v1.7.0.1.img",
            "clockwork": false
        },
        "legacy_versions": ["2.5.0.7", "2.5.1.4", "2.5.1.3", "2.5.1.1"],
        "key": "hero"
    },
    {
        "name": "HTC Hero (CDMA)",
        "version": "2.5.0.7",
        "init": "init.heroc.rc",
        "alternate_recovery": {
            "name": "RA Recovery v1.6.2",
            "url": "http://www.androidspin.com/downloads.php?dir=amon_ra/RECOVERY/&file=recovery-RA-heroc-v1.6.2.img",
            "clockwork": false
        },
        "legacy_versions": ["2.5.0.7", "2.5.1.4", "2.5.1.3", "2.5.1.1"],
        "key": "heroc"
    },
    {
        "name": "HTC Evo",
        "version": "5.0.2.2",
        "touch_version": "5.8.0.1",
        "init": "init.supersonic.rc",
        "alternate_recovery": {
            "name": "RA Recovery v2.3",
            "url": "http://files.androidspin.com/downloads.php?dir=amon_ra/RECOVERY/&file=recovery-RA-supersonic-v2.3.img",
            "clockwork": false
        },
        "legacy_versions": ["2.5.0.7", "3.0.0.5", "3.0.0.4", "3.0.0.3", "3.0.0.5", "5.0.0.0", "5.0.0.8", "5.0.1.0", "5.0.2.0", "5.0.2.2"],
        "key": "supersonic",
        "lunch": "cyanogen_supersonic-eng"
    },
    {
        "name": "HTC Legend",
        "version": "5.0.2.0",
        "readonly_recovery": true,
        "init": "init.legend.rc",
        "legacy_versions": ["2.5.0.7", "2.5.1.4", "2.5.1.3", "2.5.1.1", "2.5.0.7", "5.0.0.1", "5.0.0.8", "5.0.1.0"],
        "key": "legend",
        "lunch": "cyanogen_legend-eng"
    },
    {
        "name": "HTC Desire",
        "touch_version": "5.8.0.2",
        "version": "5.0.2.0",
        "init": "init.bravo.rc",
        "legacy_versions": ["2.5.0.7", "2.5.1.4", "2.5.1.3", "2.5.1.1", "2.5.0.7", "5.0.0.0", "5.0.0.8", "5.0.1.0"],
        "key": "bravo",
        "lunch": "cyanogen_bravo-eng"
    },
    {
        "name": "HTC Desire CDMA",
        "version": "5.0.2.0",
        "init": "init.bravoc.rc",
        "legacy_versions": ["2.5.0.9", "2.5.1.4", "2.5.1.3", "2.5.1.1", "2.5.0.9", "5.0.0.1", "5.0.1.0"],
        "key": "bravoc",
        "lunch": "cyanogen_bravoc-eng"
    },
    {
        "name": "Huawei Pulse Mini",
        "version": "2.5.0.7",
        "init": "init.qcom.rc",
        "legacy_versions": ["2.5.0.7", "2.5.1.4", "2.5.1.3", "2.5.1.1"],
        "key": "pulsemini"
    },
    {
        "name": "HTC Incredible (CDMA)",
        "version": "5.0.2.0",
        "init": "init.inc.rc",
        "alternate_recovery": {
            "name": "RA_GNM Recovery v3.02",
            "url": "http://dl.dropbox.com/u/12949506/RomManager/recovery-RA-inc-3.02.img",
            "clockwork": false
        },
        "legacy_versions": ["3.0.0.8", "3.0.0.7", "3.0.0.6", "3.0.0.5", "3.0.0.8", "5.0.0.0", "5.0.0.1", "5.0.0.8", "5.0.1.0"],
        "key": "inc",
        "lunch": "cyanogen_inc-eng"
    },
    {
        "name": "HTC Aria",
        "version": "2.5.0.7",
        "readonly_recovery": true,
        "init": "init.liberty.rc",
        "legacy_versions": ["2.5.0.7", "2.5.1.4", "2.5.1.3", "2.5.1.1"],
        "key": "liberty"
    },
    {
        "name": "Motorola Droid Eris",
        "version": "2.5.0.7",
        "readonly_recovery": true,
        "init": "init.desirec.rc",
        "alternate_recovery": {
            "name": "RA Recovery v1.6.2",
            "url": "http://www.androidspin.com/downloads.php?dir=amon_ra/RECOVERY/&file=recovery-RA-eris-v1.6.2.img",
            "clockwork": false
        },
        "legacy_versions": ["2.5.0.7", "2.5.1.4", "2.5.1.3", "2.5.1.1"],
        "key": "desirec"
    },
    {
        "name": "HTC Buzz (Wildfire)",
        "version": "5.0.2.0",
        "readonly_recovery": true,
        "init": "init.buzz.rc",
        "legacy_versions": ["3.0.0.6", "2.5.1.4", "2.5.1.3", "2.5.1.1", "2.5.0.7", "5.0.0.8", "5.0.1.0"],
        "key": "buzz",
        "lunch": "cyanogen_buzz-eng"
    },
    {
        "name": "Dell Streak",
        "version": "2.5.0.7",
        "init": "init.qcom.rc",
        "legacy_versions": ["2.5.0.7", "2.5.1.4", "2.5.1.3", "2.5.1.1"],
        "key": "streak"
    },
    {
        "name": "HTC MyTouch Slide",
        "version": "2.5.0.7",
        "readonly_recovery": true,
        "init": "init.latte.rc",
        "legacy_versions": ["2.5.0.7", "2.5.1.4", "2.5.1.3", "2.5.1.1"],
        "key": "espresso"
    },
    {
        "name": "Samsung GalaxyS i9000",
        "version": "2.5.1.2",
        "readonly_recovery": true,
        "init": "lpm.rc",
        "legacy_versions": ["2.5.1.2", "2.5.1.4", "2.5.1.3", "2.5.1.1"],
        "key": "galaxys"
    },
    {
        "name": "Samsung GalaxyS Fascinate",
        "version": "3.0.0.8",
        "readonly_recovery": true,
        "init": "init.smdkc110.rc",
        "legacy_versions": ["3.0.0.8", "2.5.1.4", "2.5.1.3", "2.5.1.1"],
        "key": "fascinate"
    },
    {
        "name": "Samsung GalaxyS Vibrant",
        "version": "2.5.1.2",
        "readonly_recovery": true,
        "init": "init.smdkc110.rc",
        "legacy_versions": ["2.5.1.2", "2.5.1.4", "2.5.1.3", "2.5.1.1"],
        "key": "vibrant"
    },
    {
        "name": "Samsung GalaxyS Captivate",
        "version": "2.5.1.2",
        "readonly_recovery": true,
        "init": "init.smdkc110.rc",
        "legacy_versions": ["2.5.1.2", "2.5.1.4", "2.5.1.3", "2.5.1.1"],
        "key": "captivate"
    },
    {
        "name": "Huawei Pulse",
        "version": "2.5.0.7",
        "init": "init.qcom.rc",
        "legacy_versions": ["2.5.0.7", "2.5.1.4", "2.5.1.3", "2.5.1.1"],
        "key": "pulse"
    },
    {
        "name": "LGE Ally",
        "version": "2.5.0.7",
        "init": "init.aloha.rc",
        "alternate_recovery": {
            "name": "RA_GNM Recovery v2.2.2",
            "url": "http://dl.dropbox.com/u/12949506/RomManager/recovery-RA-aloha-2.2.2-GNM.img",
            "clockwork": false
        },
        "legacy_versions": ["2.5.0.7", "2.5.1.4", "2.5.1.3", "2.5.1.1"],
        "key": "aloha"
    },
    {
        "name": "Motorola Milestone",
        "version": "3.1.0.2",
        "readonly_recovery": true,
        "init": "init.mapphone_cdma.rc",
        "legacy_versions": ["2.5.0.7", "2.5.1.4", "2.5.1.3", "2.5.1.1", "2.5.0.7"],
        "key": "milestone"
    },
    {
        "name": "Motorola Droid X",
        "flash_recovery": "dd if=%s of=/preinstall/recovery/recovery.zip ; ",
        "reboot_recovery": "rm /cache/recovery/command ; echo 1 > /data/.recovery_mode ; sync ; reboot ;",
        "version": "5.0.2.0",
        "readonly_recovery": true,
        "init": "init.mapphone_cdma.rc",
        "legacy_versions": ["2.5.0.7"],
        "key": "droidx"
    },
    {
        "name": "Motorola Droid X (2nd-init)",
        "flash_recovery": "dd if=%s of=/preinstall/recovery/recovery.zip ; ",
        "reboot_recovery": "rm /cache/recovery/command ; echo 1 > /data/.recovery_mode ; sync ; reboot ;",
        "version": "5.0.2.0",
        "readonly_recovery": true,
        "init": "init.mapphone_cdma.rc",
        "legacy_versions": ["2.5.0.7"],
        "key": "shadow",
        "lunch": "cyanogen_shadow-eng"
    },
    {
        "name": "Acer Liquid",
        "version": "2.5.1.0",
        "init": "init.salsa.rc",
        "legacy_versions": ["2.5.1.4", "2.5.1.3", "2.5.1.1", "2.5.1.0"],
        "key": "salsa"
    },
    {
        "name": "Commtiva Z71 (Boston, Blaze, and more)",
        "version": "2.5.1.3",
        "init": "init.qcom.rc",
        "legacy_versions": ["2.5.1.4", "2.5.1.3", "2.5.1.1", "2.5.1.0"],
        "key": "z71"
    },
    {
        "name": "HTC G2",
        "flash_recovery": "dd if=/dev/zero of=/dev/block/mmcblk0p21 bs=4096 ; dd if=%s of=/dev/block/mmcblk0p21 bs=4096 ; ",
        "touch_version": "5.8.1.0",
        "version": "5.0.2.7",
        "init": "init.vision.rc",
        "legacy_versions": ["3.0.2.4", "3.0.0.5", "2.5.1.4", "2.5.1.3", "3.0.2.4", "5.0.0.1", "5.0.0.8", "5.0.1.0", "5.0.2.0", "5.0.2.6", "5.0.2.0"],
        "key": "vision",
        "lunch": "cyanogen_vision-eng"
    },
    {
        "name": "HTC MyTouch 4G",
        "flash_recovery": "dd if=%s of=/dev/block/mmcblk0p21 ; ",
        "version": "5.0.2.0",
        "touch_version": "5.8.1.0",
        "init": "init.glacier.rc",
        "legacy_versions": ["3.0.2.4", "3.0.0.5", "2.5.1.4", "2.5.1.3", "3.0.2.4", "5.0.0.1", "5.0.0.8", "5.0.1.0"],
        "key": "glacier",
        "lunch": "cyanogen_glacier-eng"
    },
    {
        "name": "HTC Desire HD",
        "flash_recovery": "dd if=%s of=/dev/block/mmcblk0p21 ; ",
        "touch_version": "5.8.1.5",
        "version": "5.0.2.0",
        "init": "init.spade.rc",
        "legacy_versions": ["3.0.2.5", "3.0.0.6", "3.0.0.5", "2.5.1.4", "3.0.2.5", "3.0.2.6", "5.0.0.1", "5.0.0.8", "5.0.1.0"],
        "key": "ace",
        "lunch": "cyanogen_ace-eng"
    },
    {
        "name": "Motorola Cliq",
        "version": "5.0.2.0",
        "touch_version": "5.8.1.0",
        "init": "init.morrison.rc",
        "legacy_versions": ["2.5.1.2", "2.5.1.4", "2.5.1.3", "2.5.1.1", "2.5.1.2", "5.0.0.1", "5.0.0.1", "5.0.0.8", "5.0.1.0"],
        "key": "morrison",
        "lunch": "cyanogen_morrison-eng"
    },
    {
        "name": "Motorola Droid 2",
        "flash_recovery": "dd if=%s of=/preinstall/recovery/recovery.zip ; ",
        "reboot_recovery": "rm /cache/recovery/command ; echo 1 > /data/.recovery_mode ; sync ; reboot ;",
        "version": "5.0.2.0",
        "readonly_recovery": true,
        "init": "init.mapphone_cdma.rc",
        "legacy_versions": ["2.5.0.8"],
        "key": "droid2",
        "lunch": "cyanogen_droid2-eng"
    },
    {
        "name": "Geeksphone ONE",
        "version": "2.5.1.3",
        "init": "init.qcom.rc",
        "alternate_recovery": {
            "name": "ONE Recovery v1.6",
            "url": "http://dl.dropbox.com/u/10355648/recovery-ONE-1_6.img",
            "clockwork": false
        },
        "legacy_versions": ["2.5.1.4", "2.5.1.3", "2.5.1.1", "2.5.1.0"],
        "key": "one"
    },
    {
        "name": "Advent Vega",
        "flash_recovery": "/data/data/com.koushikdutta.rommanager/files/tegra_flash_image recovery %s ; ",
        "reboot_recovery": "echo start > /proc/ota ; ",
        "version": "3.0.2.7",
        "init": "init_recovery.rc",
        "legacy_versions": ["2.5.1.4", "2.5.1.3", "2.5.1.1", "2.5.1.0", "2.5.1.4"],
        "key": "vega"
    },
    {
        "name": "Viewsonic GTab (Deprecated - use smb_a1002)",
        "flash_recovery": "/data/data/com.koushikdutta.rommanager/files/tegra_flash_image recovery %s ; ",
        "version": "2.5.1.8",
        "init": "init.harmony.rc",
        "alternate_recovery": {
            "name": "ClockworkMod 2.5.1.1-bekit-0.8",
            "url": "http://dropbox.bekit.net/Android/gtablet/recovery/recovery-clockwork-2.5.1.1-bekit-0.8-harmony.img",
            "clockwork": true
        },
        "legacy_versions": ["2.5.1.8", "2.5.1.4", "2.5.1.3", "2.5.1.1"],
        "key": "harmony"
    },
    {
        "name": "Galaxy Tab - ATT",
        "version": "2.5.1.8",
        "readonly_recovery": true,
        "init": "init.smdkc110.rc",
        "legacy_versions": ["2.5.1.8", "2.5.1.4", "2.5.1.3", "2.5.1.1"],
        "key": "att_tab"
    },
    {
        "name": "Samsung Galaxy Tab - T-Mobile",
        "version": "2.5.1.8",
        "readonly_recovery": true,
        "init": "init.smdkc110.rc",
        "legacy_versions": ["2.5.1.8", "2.5.1.4", "2.5.1.3", "2.5.1.1"],
        "key": "tmobile_tab"
    },
    {
        "name": "Spice MI700, Commtiva Link N700 and more)",
        "version": "2.5.1.2",
        "init": "init.qcom.rc",
        "alternate_recovery": {
            "name": "MI700 Recovery v1.6",
            "url": "http://dl.dropbox.com/u/15816725/MI700-recovery-V1_3.img",
            "clockwork": false
        },
        "legacy_versions": ["2.5.1.4", "2.5.1.3", "2.5.1.1", "2.5.1.0"],
        "key": "FM6"
    },
    {
        "name": "Google Nexus S",
        "touch_version": "5.8.0.2",
        "version": "5.0.2.0",
        "init": "init.herring.rc",
        "legacy_versions": ["3.0.2.4", "3.0.0.5", "3.0.0.0", "2.5.1.8", "3.0.2.4", "5.0.0.0", "5.0.0.8", "5.0.1.0"],
        "key": "crespo",
        "lunch": "cyanogen_crespo-eng"
    },
    {
        "name": "Motorola Droid Pro",
        "version": "2.5.0.8",
        "readonly_recovery": true,
        "init": "init. mapphone_cdma.rc",
        "legacy_versions": ["2.5.0.8"],
        "key": "droidpro"
    },
    {
        "name": "Huawei Ascend",
        "version": "3.0.0.5",
        "init": "init.qcom.rc",
        "legacy_versions": ["3.0.0.5", "3.0.0.4"],
        "key": "ascend"
    },
    {
        "name": "ZTE Blade",
        "version": "5.0.2.0",
        "init": "init.blade.rc",
        "legacy_versions": ["3.0.1.4", "3.0.0.6", "3.0.1.4", "3.0.2.6", "3.0.2.7", "5.0.0.1", "5.0.0.8", "5.0.1.0"],
        "key": "blade",
        "lunch": "cyanogen_blade-eng"
    },
    {
        "name": "Nook Color",
        "inits": ["env.txt", "init.encore.rc"],
        "reboot_recovery": "dd if=/dev/zero of=/rom/bcb bs=64 count=1 ; echo 'recovery' >> /rom/bcb ; reboot ;",
        "flash_recovery": "busybox mount -orw,remount / ; mkdir /boot ; busybox mount /dev/block/mmcblk0p1 /boot ; busybox unzip -p %1$s ramdisk-recovery.ub > /boot/uRecRam ; busybox unzip -p %1$s kernel > /boot/uRecImg ; busybox umount /boot ; rmdir /boot; busybox mount -oro,remount / ; ",
        "version": "3.2.0.1",
        "init": "env.txt",
        "legacy_versions": ["3.0.1.0", "3.0.0.9", "3.0.0.8", "3.0.0.6", "3.0.1.0", "3.0.2.8", "3.2.0.0", "3.2.0.0", "3.2.0.1", "3.2.0.1"],
        "key": "encore"
    },
    {
        "name": "HTC Click/Tattoo",
        "version": "5.0.2.0",
        "init": "init.bahamas.rc",
        "legacy_versions": ["3.0.0.6", "3.0.0.6", "3.0.2.7", "5.0.0.8", "5.0.1.0"],
        "key": "click",
        "lunch": "cyanogen_click-eng"
    },
    {
        "name": "Elocity A7",
        "version": "3.0.0.8",
        "readonly_recovery": true,
        "init": "init_recovery.rc",
        "legacy_versions": ["3.0.0.8"],
        "key": "a7"
    },
    {
        "name": "Geeksphone Zero",
        "version": "3.0.1.3",
        "init": "init.zero.rc",
        "legacy_versions": ["3.0.1.3"],
        "key": "zero"
    },
    {
        "name": "HTC Evo Shift",
        "version": "5.0.2.0",
        "touch_version": "5.8.1.6",
        "init": "init.speedy.rc",
        "legacy_versions": ["3.0.1.4", "3.0.1.3", "3.0.1.2", "3.0.1.1", "3.0.1.4", "3.0.2.8", "3.2.0.1"],
        "key": "speedy",
        "lunch": "cyanogen_speedy-eng"
    },
    {
        "name": "Samsung Captivate (MTD)",
        "reboot_recovery": "echo 1 > /cache/.startrecovery ; sync ; reboot ;",
        "init": "init.aries.rc",
        "officially_supported": false,
        "key": "captivatemtd",
        "legacy_versions": []
    },
    {
        "name": "Samsung Showcase (MTD)",
        "reboot_recovery": "echo 1 > /cache/.startrecovery ; sync ; reboot ;",
        "init": "init.aries.rc",
        "officially_supported": false,
        "key": "showcasemtd",
        "legacy_versions": []
    },
    {
        "name": "Samsung Mesmerize (MTD)",
        "reboot_recovery": "echo 1 > /cache/.startrecovery ; sync ; reboot ;",
        "init": "init.aries.rc",
        "officially_supported": false,
        "key": "mesmerizemtd",
        "legacy_versions": []
    },
    {
        "name": "Samsung Fascinate (MTD)",
        "reboot_recovery": "echo 1 > /cache/.startrecovery ; sync ; reboot ;",
        "init": "init.aries.rc",
        "officially_supported": false,
        "key": "fascinatemtd",
        "legacy_versions": []
    },
    {
        "name": "Samsung Vibrant (MTD)",
        "reboot_recovery": "echo 1 > /cache/.startrecovery ; sync ; reboot ;",
        "init": "init.aries.rc",
        "officially_supported": false,
        "key": "vibrantmtd",
        "legacy_versions": []
    },
    {
        "name": "Samsung GalaxyS i9000B (MTD)",
        "reboot_recovery": "echo 1 > /cache/.startrecovery ; sync ; reboot ;",
        "init": "init.aries.rc",
        "officially_supported": false,
        "key": "galaxysbmtd",
        "legacy_versions": []
    },
    {
        "name": "Samsung GalaxyS i9000 (MTD)",
        "reboot_recovery": "echo 1 > /cache/.startrecovery ; sync ; reboot ;",
        "init": "init.aries.rc",
        "officially_supported": false,
        "key": "galaxysmtd",
        "legacy_versions": []
    },
    {
        "name": "Samsung Epic4G",
        "version": "5.0.2.7",
        "init": "lpm.rc",
        "readonly_recovery": true,
        "key": "epic4g",
        "lunch": "full_epic4g-eng",
        "legacy_versions": []
    },
    {
        "name": "Dell Streak 7",
        "flash_recovery": "dd if=%s of=/dev/block/mmcblk3p1 ; ",
        "version": "3.0.2.0",
        "init": "init.tinyandroid.rc",
        "legacy_versions": ["3.0.2.0", "3.0.1.9", "3.0.1.8", "3.0.1.7"],
        "key": "streak7"
    },
    {
        "name": "HTC Thunderbolt",
        "touch_version": "5.8.0.2",
        "version": "5.0.2.1",
        "init": "init.mecha.rc",
        "alternate_recovery": {
            "name": "RA_GNM Recovery v3.05",
            "url": "http://dl.dropbox.com/u/12949506/RomManager/recovery-ra-mecha-3.05.img",
            "clockwork": false
        },
        "legacy_versions": ["3.0.2.5", "3.0.2.4", "3.0.2.3", "3.0.2.2", "3.0.2.5", "3.1.0.1", "3.1.0.2", "4.0.1.5", "5.0.0.1", "5.0.0.8", "5.0.1.0", "5.0.2.0"],
        "key": "mecha",
        "lunch": "cyanogen_mecha-eng"
    },
    {
        "name": "Huawei U8150 Ideos",
        "version": "3.0.2.4",
        "init": "init.ideos.rc",
        "legacy_versions": ["3.0.2.4"],
        "key": "ideos"
    },
    {
        "name": "Motorola XOOM",
        "flash_recovery": "dd if=%s of=/dev/block/platform/sdhci-tegra.3/by-name/recovery ; ",
        "version": "4.0.0.4",
        "init": "init.stingray.rc",
        "legacy_versions": ["3.0.2.5", "3.0.2.5", "4.0.0.0", "4.0.0.0", "4.0.0.0", "4.0.0.0", "4.0.0.0", "4.0.0.0", "4.0.0.1"],
        "key": "stingray"
    },
    {
        "name": "HTC HD2",
        "version": "3.1.0.0",
        "init": "init.leo.rc",
        "legacy_versions": ["3.0.2.5", "3.0.2.4", "3.0.2.5"],
        "key": "leo"
    },
    {
        "name": "Motorola CLIQ XT/Quench (MB501)",
        "version": "5.0.2.0",
        "init": "init.zeppelin.rc",
        "legacy_versions": ["3.0.2.4", "3.0.2.4", "5.0.0.8", "5.0.0.8", "5.0.1.0", "5.0.1.0"],
        "key": "zeppelin",
        "lunch": "cyanogen_zeppelin-eng"
    },
    {
        "name": "HTC Desire S",
        "flash_recovery": "dd if=%s of=/dev/block/mmcblk0p21 ; ",
        "version": "5.0.2.0",
        "touch_version": "5.8.1.5",
        "init": "init.saga.rc",
        "legacy_versions": ["3.0.2.6", "5.0.1.9"],
        "key": "saga",
        "lunch": "full_saga-eng"
    },
    {
        "name": "LG/T-Mobile G2X",
        "reboot_recovery": "echo 'boot-recovery' | dd of=/dev/block/mmcblk0p3 seek=6144 bs=1 ; reboot ; ",
        "version": "5.0.2.0",
        "readonly_recovery": true,
        "init": "init_recovery.rc",
        "legacy_versions": ["3.0.2.7", "5.0.0.8", "5.0.1.0", "5.0.1.4", "5.0.1.6"],
        "key": "p999",
        "lunch": "cyanogen_p999-eng"
    },
    {
        "name": "LGE Optimus 2X",
        "inits": ["init_recovery.rc", "/bootimages/opening_01.rle"],
        "reboot_recovery": "echo 'boot-recovery' | dd of=/dev/block/mmcblk0p3 seek=6144 bs=1 ; reboot ; ",
        "flash_recovery": "dd if=%s of=/dev/block/mmcblk0p7 ; ",
        "version": "5.0.2.0",
        "touch_version": "5.8.1.3",
        "legacy_versions": ["3.0.2.7", "3.0.2.7", "3.0.2.8", "4.0.0.9", "5.0.0.8", "5.0.1.0", "5.0.1.4", "5.0.1.6"],
        "key": "p990",
        "lunch": "cyanogen_p990-eng"
    },
    {
        "name": "Google Nexus S 4G",
        "version": "5.0.2.0",
        "touch_version": "5.8.0.2",
        "init": "init.herring.rc",
        "legacy_versions": ["3.1.0.0", "3.1.0.0", "3.1.0.1", "5.0.0.0", "5.0.0.8", "5.0.1.0"],
        "key": "crespo4g",
        "lunch": "cyanogen_crespo4g-eng"
    },
    {
        "name": "LGE Optimus One (old BB)",
        "version": "5.0.2.0",
        "init": "init.thunderg.rc",
        "alternate_recovery": {
            "name": "RA_GNM Recovery v2.2.1",
            "url": "http://dl.dropbox.com/u/12949506/RomManager/recovery-RA-thunderg-2.2.1-GNM.img",
            "clockwork": false
        },
        "legacy_versions": ["3.1.0.0", "5.0.0.8", "5.0.1.0"],
        "key": "thunderg",
        "lunch": "full_thunderg-eng"
    },
    {
        "name": "LGE Optimus One",
        "version": "5.0.2.7",
        "init": "init.thunderg.rc",
        "legacy_versions": ["5.0.2.0"],
        "key": "p500",
        "lunch": "cyanogen_p500-eng"
    },
    {
        "name": "Huawei U8220/U8230",
        "version": "3.1.0.1",
        "init": "init.qcom.rc",
        "key": "u8220",
        "legacy_versions": []
    },
    {
        "name": "Samsung Charge",
        "version": "3.1.0.1",
        "readonly_recovery": true,
        "init": "init.smdkc110.rc",
        "key": "charge",
        "legacy_versions": []
    },
    {
        "name": "Motorola Defy",
        "init": "init.mapphone_umts.rc",
        "officially_supported": false,
        "key": "umts_jordan",
        "legacy_versions": []
    },
    {
        "name": "Samsung GalaxyS 4G",
        "version": "3.2.0.0",
        "readonly_recovery": true,
        "init": "init.smdkc110.rc",
        "legacy_versions": ["3.2.0.0"],
        "key": "galaxys4g"
    },
    {
        "name": "Samsung GalaxyS2",
        "inits": ["init.smdkc210.rc", "init.smdk4210.rc"],
        "touch_version": "5.8.1.5",
        "version": "4.0.1.5",
        "readonly_recovery": true,
        "init": "init.smdkc210.rc",
        "legacy_versions": ["4.0.1.4", "v4.0.1.4"],
        "key": "galaxys2"
    },
    {
        "name": "Samsung Sidekick 4G",
        "version": "4.0.0.2",
        "readonly_recovery": true,
        "init": "init.smdkc110.rc",
        "key": "sidekick4g",
        "legacy_versions": []
    },
    {
        "name": "HTC Incredible S",
        "flash_recovery": "dd if=%s of=/dev/block/mmcblk0p21 ; ",
        "version": "5.0.2.0",
        "init": "init.vivo.rc",
        "legacy_versions": ["4.0.0.4", "4.0.0.6", "4.0.0.7", "4.0.0.8", "5.0.1.9"],
        "key": "vivo",
        "lunch": "cyanogen_vivo-eng"
    },
    {
        "name": "HTC Incredible 2",
        "flash_recovery": "dd if=%s of=/dev/block/mmcblk0p21 ; ",
        "version": "5.0.2.0",
        "init": "init.vivow.rc",
        "alternate_recovery": {
            "name": "RA_GNM Recovery v3.05",
            "url": "http://dl.dropbox.com/u/12949506/RomManager/recovery-ra-vivow-3.05.img",
            "clockwork": false
        },
        "legacy_versions": ["4.0.0.4", "4.0.0.6", "4.0.0.7", "4.0.0.8", "5.0.1.9"],
        "key": "vivow",
        "lunch": "cyanogen_vivow-eng"
    },
    {
        "name": "LGE Optimus V",
        "version": "3.2.0.1",
        "init": "init.cappuccino.rc",
        "key": "thunderc",
        "legacy_versions": []
    },
    {
        "name": "LGE Optimus 3D",
        "inits": ["init.p920.rc", "/bootimages/opening_01.rle"],
        "reboot_recovery": "echo '518 R 1' > /sys/kernel/kobject_lge_nvdata/dynamic_nvdata_raw_write ; reboot ; ",
        "flash_recovery": "dd if=%s of=/dev/block/mmcblk0p6 ; ",
        "version": "5.0.2.7",
        "touch_version": "5.8.1.5",
        "legacy_versions": ["4.0.0.7", "4.0.0.9", "5.0.1.4", "5.0.2.0"],
        "key": "p920",
        "lunch": "p920-eng"
    },
    {
        "name": "Samsung Galaxy Ace",
        "flash_recovery": "/data/data/com.koushikdutta.rommanager/files/flash_image recoveryonly %s ; ",
        "version": "4.0.0.9",
        "init": "init.galaxyace.rc",
        "key": "galaxyace",
        "legacy_versions": []
    },
    {
        "name": "Motorola Atrix 4G",
        "inits": ["init_prep_keypad.sh", "init.olympus.rc"],
        "flash_recovery": "dd if=%s of=/dev/block/mmcblk0p10 ; ",
        "touch_version": "5.8.1.8",
        "version": "5.0.2.0",
        "init": "init_prep_keypad.sh",
        "legacy_versions": ["4.0.1.0", "4.0.1.1", "3.2.0.1", "4.0.1.2", "4.0.1.3", "4.0.1.4", "4.0.1.5", "5.0.0.1", "5.0.0.1", "5.0.0.5", "5.0.0.6", "5.0.0.7", "5.0.1.0"],
        "key": "olympus",
        "lunch": "generic_olympus-eng"
    },
    {
        "name": "Viewsonic GTablet (smb_a1002) - Bootloader 1.1",
        "flash_recovery": "/data/data/com.koushikdutta.rommanager/files/tegra_flash_image recovery %s ; ",
        "version": "4.0.1.5",
        "init": "init_recovery.rc",
        "key": "smb_a1002",
        "legacy_versions": []
    },
    {
        "name": "HTC EVO 3D",
        "flash_recovery": "dd if=%s of=/dev/block/mmcblk0p22 ; ",
        "version": "5.0.2.0",
        "touch_version": "5.8.0.2",
        "init": "init.shooter.rc",
        "legacy_versions": ["4.0.1.5", "5.0.0.0", "5.0.1.0"],
        "key": "shooter",
        "lunch": "htc_shooter-eng"
    },
    {
        "name": "HTC Sensation",
        "flash_recovery": "dd if=%s of=/dev/block/mmcblk0p21 ; ",
        "version": "5.0.2.0",
        "touch_version": "5.8.0.9",
        "init": "init.pyramid.rc",
        "legacy_versions": ["4.0.1.5", "5.0.0.1", "5.0.0.7", "5.0.0.8", "5.0.1.0"],
        "key": "pyramid",
        "lunch": "full_pyramid-eng"
    },
    {
        "name": "Huawei Ascend 2",
        "version": "4.0.1.5",
        "init": "init.qcom.rc",
        "key": "ascend2",
        "legacy_versions": []
    },
    {
        "name": "Micromax A70",
        "version": "4.0.1.5",
        "init": "init.qcom.rc",
        "key": "a70",
        "legacy_versions": []
    },
    {
        "name": "Samsung Galaxy Gio",
        "version": "4.0.1.5",
        "readonly_recovery": true,
        "init": "init.galaxygio.rc",
        "key": "galaxygio",
        "legacy_versions": []
    },
    {
        "name": "Samsung Indulge 910",
        "version": "4.0.1.5",
        "readonly_recovery": true,
        "init": "init.smdkc110.rc",
        "key": "indulge910",
        "legacy_versions": []
    },
    {
        "name": "Samsung Indulge 915",
        "version": "4.0.1.5",
        "readonly_recovery": true,
        "init": "init.smdkc110.rc",
        "key": "indulge915",
        "legacy_versions": []
    },
    {
        "name": "LGE Optimus Black",
        "flash_recovery": "dd if=%s of=/dev/block/mmcblk0p4 ; ",
        "touch_version": "5.8.1.3",
        "version": "5.0.2.6",
        "init": "/bootimages/ON_480x800_08fps_0000.rle",
        "legacy_versions": ["5.0.0.0", "5.0.0.1", "5.0.0.8", "5.0.1.0", "5.0.1.1", "5.0.1.1", "5.0.1.2", "5.0.1.3", "5.0.2.0"],
        "key": "p970",
        "lunch": "p970-eng"
    },
    {
        "name": "HTC EVO 3D (GSM)",
        "flash_recovery": "dd if=%s of=/dev/block/mmcblk0p21 ; ",
        "version": "5.0.2.0",
        "touch_version": "5.8.0.2",
        "init": "init.shooteru.rc",
        "legacy_versions": ["5.0.1.6", "5.0.1.7", "5.0.1.8", "5.0.1.9"],
        "key": "shooteru",
        "lunch": "full_shooteru-eng"
    },
    {
        "name": "Motorola Droid 2 Global",
        "flash_recovery": "dd if=%s of=/preinstall/recovery/recovery.zip ; ",
        "reboot_recovery": "rm /cache/recovery/command ; echo 1 > /data/.recovery_mode ; sync ; reboot ;",
        "version": "5.0.2.3",
        "readonly_recovery": true,
        "init": "init.mapphone_cdma.rc",
        "key": "droid2we",
        "lunch": "cyanogen_droid2we-eng",
        "legacy_versions": []
    },
    {
        "name": "Motorola Droid Bionic",
        "flash_recovery": "dd if=%s of=/preinstall/recovery/recovery.zip ; ",
        "reboot_recovery": "rm /cache/recovery/command ; echo 1 > /data/.recovery_mode ; sync ; reboot ;",
        "version": "5.0.2.5",
        "readonly_recovery": true,
        "init": "init.mapphone_cdma.rc",
        "legacy_versions": ["5.0.2.3", "5.0.2.4"],
        "key": "targa",
        "lunch": "generic_targa-eng"
    },
    {
        "name": "Motorola Droid X2",
        "flash_recovery": "dd if=%s of=/preinstall/recovery/recovery.zip ; ",
        "reboot_recovery": "rm /cache/recovery/command ; echo 1 > /data/.recovery_mode ; sync ; reboot ;",
        "version": "5.0.2.5",
        "readonly_recovery": true,
        "init": "init.mapphone_cdma.rc",
        "legacy_versions": ["5.0.2.3", "5.0.2.4"],
        "key": "daytona",
        "lunch": "generic_daytona-eng"
    },
    {
        "name": "Motorola Droid 3",
        "flash_recovery": "dd if=%s of=/preinstall/recovery/recovery.zip ; ",
        "reboot_recovery": "rm /cache/recovery/command ; echo 1 > /data/.recovery_mode ; sync ; reboot ;",
        "version": "5.0.2.6",
        "readonly_recovery": true,
        "init": "init.mapphone_cdma.rc",
        "legacy_versions": ["5.0.2.5"],
        "key": "solana",
        "lunch": "full_solana-eng"
    },
    {
        "name": "Motorola Backflip",
        "version": "5.0.2.6",
        "init": "init.motus.rc",
        "key": "motus",
        "lunch": "cyanogen_motus-eng",
        "legacy_versions": []
    },
    {
        "name": "Samsung Galaxy Mini",
        "version": "5.0.2.6",
        "init": "init.gt-s5570.rc",
        "readonly_recovery": true,
        "key": "tass",
        "lunch": "tass-eng",
        "legacy_versions": []
    },
    {
        "name": "Huawei U8800",
        "flash_recovery": "mount -o remount,rw -t vfat /dev/block/mmcblk0p1 /cust_backup ; cat %s > /cust_backup/image/recovery.img ;",
        "version": "5.0.2.6",
        "init": "init.u8800.rc",
        "key": "u8800",
        "lunch": "huawei_u8800-eng",
        "legacy_versions": []
    },
    {
        "name": "Samsung AT&T GalaxyS2",
        "inits": ["init.smdkc210.rc", "init.smdk4210.rc"],
        "touch_version": "5.8.1.5",
        "version": "5.0.2.6",
        "readonly_recovery": true,
        "init": "init.smdkc210.rc",
        "key": "galaxys2att",
        "lunch": "cyanogen_galaxys2att-eng",
        "legacy_versions": []
    },
    {
        "name": "Samsung T-Mobile GalaxyS2",
        "flash_recovery": "dd if=%s of=/dev/block/mmcblk0p22 bs=4096 ; ",
        "version": "5.0.2.7",
        "touch_version": "5.8.1.3",
        "init": "init.target.rc",
        "legacy_versions": ["5.0.2.6"],
        "key": "hercules",
        "lunch": "full_hercules-eng"
    },
    {
        "name": "Samsung Epic 4G Touch",
        "version": "5.0.2.7",
        "touch_version": "5.8.1.5",
        "readonly_recovery": true,
        "init": "init.smdkc210.rc",
        "legacy_versions": ["5.0.2.6", "5.0.2.7"],
        "key": "epic4gtouch",
        "lunch": "full_epic4gtouch-eng"
    },
    {
        "name": "HTC Amaze",
        "flash_recovery": "dd if=%s of=/dev/block/mmcblk0p23 bs=4096 ; ",
        "version": "5.0.2.6",
        "init": "init.ruby.rc",
        "key": "ruby",
        "lunch": "full_ruby-eng",
        "legacy_versions": []
    },
    {
        "name": "Samsung Galaxy 5",
        "version": "5.0.2.8",
        "init": "init.gt-i5500.rc",
        "readonly_recovery": true,
        "legacy_versions": ["5.0.2.8"],
        "key": "galaxy5",
        "lunch": "samsung_galaxy5-eng"
    },
    {
        "name": "ZTE Racer",
        "version": "5.0.2.7",
        "init": "init.mooncake.rc",
        "key": "mooncake",
        "lunch": "generic_mooncake-eng",
        "legacy_versions": []
    },
    {
        "name": "Samsung Galaxy Tab - VZW",
        "version": "5.0.2.7",
        "init": "init.smdkc110.rc",
        "readonly_recovery": true,
        "legacy_versions": ["5.0.2.7", "5.0.2.7", "5.0.2.7"],
        "key": "vzwtab",
        "lunch": "full_vzwtab-eng"
    },
    {
        "name": "HTC MyTouch 4G Slide",
        "flash_recovery": "dd if=%s of=/dev/block/mmcblk0p21 bs=4096 ; ",
        "version": "5.0.2.7",
        "init": "init.doubleshot.rc",
        "legacy_versions": ["5.0.2.7", "5.0.2.7"],
        "key": "doubleshot",
        "lunch": "full_doubleshot-eng"
    },
    {
        "name": "Google Galaxy Nexus (GSM)",
        "flash_recovery": "dd if=%s of=/dev/block/platform/omap/omap_hsmmc.0/by-name/recovery bs=4096 ;",
        "version": "5.5.0.2",
        "touch_version": "5.8.0.2",
        "init": "init.tuna.rc",
        "legacy_versions": ["5.5.0.0", "5.5.0.1"],
        "key": "maguro",
        "lunch": "full_maguro-userdebug"
    },
    {
        "name": "Google Galaxy Nexus (Verizon)",
        "flash_recovery": "dd if=%s of=/dev/block/platform/omap/omap_hsmmc.0/by-name/recovery bs=4096 ;",
        "version": "5.5.0.4",
        "touch_version": "5.8.0.2",
        "init": "init.tuna.rc",
        "legacy_versions": ["5.5.0.2", "5.5.0.3"],
        "key": "toro",
        "lunch": "full_toro-eng"
    },
    {
        "name": "LG Optimus C LW690",
        "version": "5.0.2.7",
        "init": "init.thunderc.rc",
        "key": "lg690",
        "lunch": "full_lg690-eng",
        "legacy_versions": []
    },
    {
        "name": "LGE Optimus Pro",
        "version": "5.0.2.7",
        "init": "init.muscat.rc",
        "key": "c660",
        "lunch": "c660-eng",
        "legacy_versions": []
    },
    {
        "name": "LGE Optimus Hub",
        "version": "5.0.2.7",
        "init": "init.univa.rc",
        "key": "e510",
        "lunch": "e510-eng",
        "legacy_versions": []
    },
    {
        "name": "Samsung Epic4G (MTD)",
        "reboot_recovery": "/system/xbin/busybox devmem 0x57fff800 32 0x5EC0B007; sync ; reboot ;",
        "touch_version": "5.8.1.5",
        "version": "5.0.2.7",
        "init": "init.victory.rc",
        "officially_supported": false,
        "key": "epicmtd",
        "lunch": "cyanogen_epicmtd-eng",
        "legacy_versions": []
    },
    {
        "name": "HTC Holiday",
        "flash_recovery": "dd if=%s of=/dev/block/mmcblk0p23 bs=4096 ; ",
        "version": "5.0.2.7",
        "init": "init.holiday.rc",
        "legacy_versions": ["5.0.2.7", "5.0.2.7", "5.0.2.7"],
        "key": "holiday",
        "lunch": "full_holiday-eng"
    },
    {
        "name": "LG Optimus SOL / TMo LG myTouch",
        "inits": ["init.victor.rc", "init.e739.rc"],
        "flash_recovery": "dd if=%s of=/dev/block/mmcblk0p17 bs=4096 ;",
        "version": "5.0.2.7",
        "key": "e730",
        "lunch": "e730-eng",
        "legacy_versions": []
    },
    {
        "name": "HTC Wildfire S",
        "version": "5.0.2.8",
        "init": "init.marvel.rc",
        "legacy_versions": ["5.0.2.7"],
        "key": "marvel",
        "lunch": "generic_marvel-eng"
    },
    {
        "name": "Motorola Photon 4G",
        "inits": ["init.mapphone_umts.rc", "init.mapphone_cdma.rc"],
        "flash_recovery": "dd if=%s of=/dev/block/mmcblk0p10 bs=4096 ;",
        "version": "5.0.2.8",
        "init": "init.mapphone_umts.rc",
        "legacy_versions": ["5.0.2.7"],
        "key": "sunfire",
        "lunch": "full_sunfire-eng"
    },
    {
        "name": "LGE Esteem",
        "inits": ["init.qcom.rc", "init.bryce.rc"],
        "flash_recovery": "dd if=%s of=/dev/block/mmcblk0p14 bs=4096 ;",
        "version": "5.0.2.8",
        "init": "init.qcom.rc",
        "key": "esteem",
        "lunch": "full_esteem-eng",
        "legacy_versions": []
    },
    {
        "name": "Huawei U8160",
        "inits": ["init.huawei.rc", "init.u8160.rc"],
        "version": "5.0.2.8",
        "init": "init.u8160.rc",
        "legacy_versions": ["5.0.2.8"],
        "key": "u8160",
        "lunch": "huawei_u8160-eng"
    },
    {
        "name": "HTC Rezound",
        "flash_recovery": "dd if=%s of=/dev/block/mmcblk0p23 bs=4096 ;",
        "touch_version": "5.8.1.4",
        "version": "5.8.1.4",
        "init": "init.vigor.rc",
        "legacy_versions": ["5.5.0.4", "5.5.0.5", "5.8.1.3", "5.8.1.4", "5.8.1.4", "5.8.1.4"],
        "key": "vigor",
        "lunch": "full_vigor-userdebug"
    },
    {
        "name": "HTC EVO View 4G",
        "flash_recovery": "dd if=%s of=/dev/block/mmcblk0p21 bs=4096 ;",
        "version": "5.8.1.5",
        "init": "init.expresskt.rc",
        "key": "express",
        "lunch": "full_express-userdebug",
        "legacy_versions": []
    },
    {
        "name": "Asus Transformer Prime",
        "flash_recovery": "dd if=%s of=/dev/block/mmcblk0p4 ;",
        "version": "5.8.2.0",
        "touch_version": "5.8.1.8",
        "init": "init.cardhu.rc",
        "legacy_versions": ["5.5.0.4"],
        "key": "tf201",
        "lunch": "cm_tf201-userdebug"
    },
    {
        "name": "Motorola Droid 4",
        "reboot_recovery": "rm /cache/recovery/command ; echo 1 > /data/.recovery_mode ; sync ; reboot ;",
        "version": "5.0.2.0",
        "init": "init.mapphone_cdma.rc",
        "key": "maserati",
        "officially_supported": false,
        "legacy_versions": []
    },
    {
        "name": "Motorola RAZR",
        "reboot_recovery": "rm /cache/recovery/command ; echo 1 > /data/.recovery_mode ; sync ; reboot ;",
        "version": "5.0.2.0",
        "init": "init.mapphone_cdma.rc",
        "key": "spyder",
        "officially_supported": false,
        "legacy_versions": []
    },
    {
        "name": "HTC Wildfire S CDMA",
        "version": "5.0.2.8",
        "init": "init.marvelc.rc",
        "key": "marvelc",
        "lunch": "full_marvelc-eng",
        "legacy_versions": []
    },
    {
        "name": "Asus Transformer",
        "flash_recovery": "dd if=%s of=/dev/block/mmcblk0p4 ;",
        "touch_version": "5.8.2.1",
        "version": "5.8.2.1",
        "init": "init.ventana.rc",
        "legacy_versions": ["5.5.0.4", "5.8.1.9", "5.8.2.0"],
        "key": "tf101",
        "lunch": "cm_tf101-userdebug"
    },
    {
        "name": "AT&T Nitro HD / LGE P930",
        "flash_recovery": "dd if=%s of=/dev/block/mmcblk0p13 ;",
        "version": "5.8.2.0",
        "init": "bootimages/boot_logo_00000.rle",
        "key": "p930",
        "lunch": "full_p930-userdebug",
        "legacy_versions": []
    },
    {
        "name": "Samsung GalaxyS2 (SGH-I777)",
        "inits": ["init.smdkc210.rc", "init.smdk4210.rc"],
        "version": "5.8.1.8",
        "readonly_recovery": true,
        "init": "init.smdkc210.rc",
        "key": "i777",
        "lunch": "cm_i777-userdebug",
        "legacy_versions": []
    },
    {
        "name": "HTC Pico",
        "version": "5.0.2.8",
        "init": "init.pico.rc",
        "key": "pico",
        "lunch": "full_pico-eng",
        "legacy_versions": []
    },
    {
        "name": "HTC One X (GSM)",
        "flash_recovery": "dd if=%s of=/dev/block/mmcblk0p5 bs=4096 ;",
        "version": "5.8.3.1",
        "touch_version": "5.8.3.1",
        "init": "ueventd.endeavoru.rc",
        "legacy_versions": ["5.8.2.6", "5.8.2.7", "5.8.2.6", "5.8.2.6", "5.8.2.8", "5.8.2.9", "5.8.3.0"],
        "key": "endeavoru",
        "lunch": "cm_endeavoru-userdebug"
    },
    {
        "name": "HTC One S",
        "flash_recovery": "dd if=%s of=/dev/block/mmcblk0p22 bs=4096 ;",
        "version": "5.8.2.3",
        "touch_version": "5.8.2.4",
        "init": "init.ville.rc",
        "legacy_versions": ["5.5.0.4", "5.8.2.1", "5.8.2.2"],
        "key": "ville",
        "lunch": "cm_ville-userdebug"
    },
    {
        "name": "Samsung Galaxy Tab (MTD and GSM)",
        "reboot_recovery": "echo 1 > /cache/.startrecovery ; sync ; reboot ;",
        "version": "5.8.2.1",
        "init": "init.gt-p1000.rc",
        "key": "p1",
        "officially_supported": false,
        "legacy_versions": []
    },
    {
        "name": "Samsung Galaxy Tab (MTD and CDMA)",
        "reboot_recovery": "echo 1 > /cache/.startrecovery ; sync ; reboot ;",
        "version": "5.8.2.1",
        "init": "init.sch-i800.rc",
        "key": "p1c",
        "officially_supported": false,
        "legacy_versions": []
    },
    {
        "name": "Google Galaxy Nexus (Sprint)",
        "flash_recovery": "dd if=%s of=/dev/block/platform/omap/omap_hsmmc.0/by-name/recovery bs=4096 ;",
        "touch_version": "5.8.3.1",
        "version": "5.8.3.1",
        "init": "init.tuna.rc",
        "key": "toroplus",
        "lunch": "cm_toroplus-userdebug",
        "legacy_versions": []
    },
    {
        "name": "Samsung Galaxy Tab 10.1",
        "flash_recovery": "dd if=%s of=/dev/block/mmcblk0p2 ; ",
        "init": "ueventd.p3.rc",
        "version": "5.8.3.1",
        "touch_version": "5.8.3.1",
        "key": "p4wifi",
        "lunch": "cm_p4wifi-userdebug",
        "legacy_versions": ["4.0.0.4"]
    }],
    "use_in_app": false,
    "pontiflex_startup_register": 100,
    "pontiflex_downloading": 100,
    "manifest": "http://developer.clockworkmod.com/merge",
    "version": "2.5.0.1",
    "recovery_url": "http://download.clockworkmod.com/recoveries/recovery-clockwork-%s-%s.img",
    "recovery_zip_url": "http://download.clockworkmod.com/recoveries/recovery-clockwork-%s-%s.zip",
    "pontiflex_startup_ad": 100,
    "ad_network": "admob",
    "keywords": "phone accessories,droid,gadgets,bluetooth headset,car charger,sd card",
    "manifest_signature": "O8+Kc2mdQz0e/AjCQn6Rw6fK5Kr5LZItJ474Ua3gF2o4IgdCQZXIbwCHIo6xeqY3pVEGtjYmmyO1PkRYcGqq2rB2wQM8SoKFl/70vG0ihEZ5TDeruKUgxVVSNM3e09kBvCxYK89GWqVWkNHtvTu1T1F9w6FilzCsNFSAXHD0Bos="
}